...
const dateAEST = date => {
  return momentTimezone(date).tz('Asia/Kuala_Lumpur')
}
...
router.put('/rooms/:id', requireJWT, (req, res) => {
  ...
  if (req.body.recurring.length === 0) {
    ...
    let bookingDateTracker = momentTimezone(firstBooking.bookingStart).tz('Asia/Kuala_Lumpur')
    ...
    let lastBookingDate = momentTimezone(firstBooking.recurring[0]).tz('Asia/Kuala_Lumpur')
    ...
    for (let i = 0; i < bookingsInRange; i++) {
      ...
      if (proposedBookingDateStart.day() !== 0) {
        ...
        let firstBookingEndDate = momentTimezone(firstBooking.bookingEnd).tz('Asia/Kuala_Lumpur')
